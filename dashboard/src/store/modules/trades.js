...
const store = {
    ...
    mutations: {
        ADD_COIN_PAIR: (state, payload) => {
            const index = state.trades.findIndex(s => s.symbol === payload.symbol);
            if (index === -1) {
                state.trades.push(payload);
                localStorage.setItem('vue-crypto-trades-new', JSON.stringify(state.trades));
            }
        },
        REMOVE_COIN_PAIR: (state, symbol) => {
            state.trades.splice(state.trades.findIndex(s => s.symbol === symbol), 1);
            localStorage.setItem('vue-crypto-trades-new', JSON.stringify(state.trades));
        },
