    ...
    <div class="menu-bar">
        <div class="selector">
            ...
            <button class="add-btn" @click="change">
                <i class="fa fa-plus fa-lg plus-icon" aria-hidden="true" />
            </button>
        </div>
        <div class="sentiment">
            <span v-if="isHodlView">Total Portfolio: {{ totalPortfolio }}</span>
            <span v-else-if="isTradesView">Total Trade Profit: {{ totalTradeProfit }}</span>
    ...
    computed: {
        ...
        totalPortfolio() {
            const total = this.hodlCurrencies.reduce((acc, hc) => {
                acc += get(this.tickers, [hc.symbol, 'price'], 0) * get(hc, 'amountOwned', 0);
                return acc;
            }, 0);
        ...
        totalTradeProfit() {
            const total = this.trades.reduce((acc, trade) => {
                ...
                const currentValue =
                    get(trade, 'amountOwned', 0) * get(this.tickers, [trade.symbol, 'price'], 0);
